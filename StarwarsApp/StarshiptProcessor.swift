...
struct StarshipProcessor: Processor {
    ...
    func process(_ filter: String) throws -> (Int, [QueryItem]) {
        ...
        var page = 1
        repeat {
            let nextResult = try networkController.search(queryType: Starships.self, filter: filter, page: page)
        ...
        return (result.count, result.results.map { QueryItem.starship($0) })
    }
}
